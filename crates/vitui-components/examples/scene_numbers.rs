...
//!    red**: components ticket 11 built the collection's screens, four of them wait for
//!    `collection` and the wheel gate waits for the fix; components ticket 21 built the accordion
//!    and the fold set, and both wait for `collapsible`. `examples/listing_numbers.rs` and
//!    `examples/collapsible_numbers.rs` are where those seven are measured.
...
fn main() {
    ...
    assert_eq!(
        ...
        "components ticket 11's five — four pinned to components 12 and one to components 20 — \
         and components ticket 21's two, both pinned to components 22"
    );
}
