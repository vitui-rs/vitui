...
//! `tests::forty_five_rows_are_evaluated_and_the_rest_say_why_not` is what makes the next change a
...
/// The overlay family's own file, which is where components ticket 25's four rows run.
    ...
    Row {
        ...
        standing: Standing::Red {
            by: &[
                ...
                Instrument::Unit {
                    file: "crates/vitui-components/src/scenes.rs",
                    name: "twenty_scenes_have_nothing_to_run_over_six_are_red_and_three_are_\
                           stood_up",
                },
            ],
...
mod tests {
    ...
    fn forty_five_rows_are_evaluated_and_the_rest_say_why_not() {
        ...
        assert_eq!(
            ...
            "the five gates that are red and pinned: the sentinel, the palette after a swap, \
             ...
             overlay family's one waiting for its two. The glyph-set count was one of them and \
    ...
    fn an_allocation_row_says_it_is_a_total() {
        ...
        // **Row 69 is the second row that had to say it**, and it is the more interesting case:
        // its claim is *marginal* — one more overlay is one more allocation — which reads like a
        // per-frame figure and is asserted as an equality on the total. A margin taken between two
        // means would be the division refinement 2 forbids, arriving as a subtraction.
        assert!(
            rows.iter().filter(|r| r.gate.contains("total")).count() >= 2,
            "a row whose claim is per-frame still measures a total, and says so"
        );
    }
