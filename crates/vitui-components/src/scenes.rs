//! The scene list, **as a normative value rather than an appendix**.
//!
...
//!
//! The criterion reads *all twenty-six*. The table is
...
//!
//! # There are thirty rows, and the three that are not the say so in a field
//!
//! The table is closed at twenty-seven and this list is longer, because **the backlog asks for
//! scenes the rules do not carry**: the narrow axis is one, the *narrow collection*
//! beside the four collection rows is the second, `issues/14`'s *equality under a horizontal
//! offset* — carried as a **register** row rather than as a scene — is the third, and
...
//! one-line diff pretending to be normative.
//!
//! The criterion says *including the three marked as owed*. **One row carries `(owed)`** —
...
//!
//! **The assembled gallery stood its scene up — and it is the first row here
...
//!
//! **The last three were taken**, having been moved off `Unsubjected`
...
//!
//! **Those three rows moved off `Unsubjected` and onto `Red`**: they now run
...
//! pinned to **components 32** with that as the failing set. It is the move one ticket
...
//!
//! **Two more moved**, the fold anchor and the accordion, and both are waiting for
...
//! **The fourteenth turned**, and it is the second to make that move for
//! the same reason: scene 14 runs a great deal — the five configurations of the table with every
...
//! not got is `select` and `overlay`. Pinned to **components 26**. Two of the seven columns do not
//! reproduce and both are recorded rather than bent: `allocations`, whose zero was the bump arena
//! was deleted, and `content layers`, whose 2 / 4 / 3 counted a shadow layer
//! `OverlayOpts` has no field for.
//!
//! **Four rows moved off `Unsubjected` and a fifth arrived**, and the direction
...
//! verdict and the job — **and it was done**: `crate::listing::draw_into`'s
...
//!
//! **Two more were pinned**, the pinned-column pair, and they are pinned to **the
...
//! [`crate::grid`] and what it found is that *no gate left by C01, C02 or C03 sees it* is half
...
//! passes wrote anyway. The criterion said so — *every scene of that set is green
//! except the wheel gate, which stays pinned red* — and the count test asserts the
...
//!
//! **Two more moved, and they are a third subject rather than a third
...
//!
//! **Three scenes are `Evaluated`, and the four components are what moved them.** They were
//! `Red` for one pass, which is the state worth reading this file for: the dense screen was built
//! screen out of the three helpers that ship — `fit`, `block` and `press` — so the 338 regions, the
//! metric row, the equality against the naive twin at both sizes and all five of
...
//! so the three rows were pinned in their failing state with that as the exact failing set —
//! own rule for a red gate, *it asserts its exact failing set, fires in both directions, and says
//! what to invert*.
//!
//! `text`, `chip`, `button` and `panel` were declared, and [`crate::dense`]'s screen is now
...
//! [`crate::dense::CHIP_FILLED_FACE`] is **1 095** where 1 149 was measured, because a chip's
...
//!
//! Every row carries what it decided, in the figures. The rule at the top is *a scene is
...
/// The screen a scene is played on, in the terms.
///
/// **[`Size::Unstated`] is an arm rather than a default.** Seventeen of the twenty-seven rows state
/// no `w x h`, and inventing one would put a number in a normative list that no ticket wrote — which
/// is the failure mode recorded for prose and inherited here for numbers. The eleven rows
/// that do state one — ten of them — are three at one size, five at two, two
/// over a domain of pairs and two in cells with no dimensions; `examples/scene_numbers.rs` prints
/// the split.
///
/// **The overlay scene moved off `Unstated` and it is not an invention**: the original's
/// row states no size, and *The frame* states one in the same sentence as the counts the row
...
pub enum Size {
    ...
    /// Two, and the scene **is** the difference between them. The overlap is green at 300x80 and
    /// red at 60x20.
    Two {
    ...
    /// A cell count with no dimensions. The recorded figure is **53 280** for the assembled gallery and no
    ...
    /// No size is stated for this row.
    Unstated,
}
...
pub enum Content {
    ...
    Assembled {
        /// How many parts are counted on the screen — interactive regions for the dense screen,
        /// panels for the gallery, sections for the accordion. **The unit is per row**,
    ...
    ///
    /// The arm the document screens add, and it is an arm rather than a reuse of
    /// [`Content::Fields`] because a corpus is not a screen: *every number and every gate
    ...
    Corpus {
        /// How many clusters.
        clusters: u32,
        /// How many kinds of cluster. Seven of them are the list.
        kinds: u8,
    },
}
...
///
/// **[`Gesture::Shrink`] and [`Gesture::Resize`] are two gestures and not one spelling of one.** The rules
...
pub struct Scene {
    /// Its row in the table, which is how everything else refers to it.
    pub number: u8,
    /// Whether it is a row of the twenty-seven-row table, or one a ticket on this backlog
    /// wrote beside it.
    ///
    /// **[`crate::gates::Row::on_spec_table`]'s arrangement, and it arrived here for its reason.**
    /// The table is closed and the count test reads it off the spec file; a scene a ticket adds
    /// has to say which side of the line it is on rather than moving that count. The backlog needs
    /// the field: the criterion asks for **five** collection scenes and
    /// four of them are carried, and the narrow axis is not a row at all.
    ...
    /// The property it decided, with the figures.
    pub decided: &'static str,
    /// The `(component, axis)` pairs it is evidence for. **O5's population, from the other side.**
    ///
    /// Claimed only where the row names the component and the axis's mechanism. A scene that
    ...
    /// **This row is marked `(owed)`.** The scene is normative and its number was never measured.
    ///
...
/// The dense screen's own file, which is where the three scenes are played.
const DENSE: &str = "crates/vitui-components/src/dense.rs";

/// The listing's own file, which is where the five scenes are played.
const LISTING: &str = "crates/vitui-components/src/listing.rs";

/// The scroll area's own file, which is where the four scenes are played.
const AREA: &str = "crates/vitui-components/src/area.rs";

/// **The three scrolling components' own file**, which is where the gates live.
...
/// **What stands the four scenes up**, and it is the same three instruments on
...
///
/// **All three were inverted and none was deleted.** The verdict now reads
...
/// The three components of F3 scrolling, which all four of the scenes wait on.
const SCROLLING: &[&str] = &["scroll_area", "scrollbar", "sticky"];

/// The forest's own file, which is where the two scenes are played.
const FOREST: &str = "crates/vitui-components/src/forest.rs";
/// The grid's own file, which is where the two scenes are played.
const GRID: &str = "crates/vitui-components/src/grid.rs";
/// The document's own file, which is where the three scenes are played.
...
/// **The field's window's own file**, which is where production the three scenes are
/// played. `crate::document` is where its fourth is.
const WINDOW: &str = "crates/vitui-components/src/window.rs";

/// **What stands production the three scenes up**, and it is the same two instruments on
...
/// **The dropped list's own file**, which is where production the four scenes are played.
/// `crate::popup` is where the overlay family's own scene 14 is.
const DROPPED: &str = "crates/vitui-components/src/dropped.rs";

/// **What stands production the four scenes up**, and it is the same two instruments on
...
/// **The scroll area's furniture's own file**, which is where production the two screens
/// are played. [`AREA`] is where this family's other four scenes are.
const SURROUND: &str = "crates/vitui-components/src/surround.rs";

/// **What stands production the two screens up**, and it is the same two instruments on
...
/// rather than a screen of its own — the arrangement for `table`, and its reason: a
...
/// screen of its own — the arrangement for `table` and 09's for the pane, and its
/// reason: a notch driven anywhere else would be a fifth copy of one drive loop. So
/// [`STANDS_ON_TREE`]'s *second* is not here — the waiting message is the original's and 9's, and this
...
/// has not played `Copy`-only body, and a scene claiming this component's wheel axis owes both.
...
///
/// **Two and not three**, and the missing one is the keyboard question: an open picker seats no
...
///
/// **Two aliases and not one, which is a review finding on the first draft.** All four
...
/// own documentation forbids by name — and the analogous pair uses a one-component
...
/// two pins claim `table` stands on their screen, and the failing set they are
...
/// **The pair every one of the three scenes rests on.**
///
...
/// The series screen's own file, which is where the two scenes are played.
...
/// **The pair both of the scenes rest on**, written once because it is one fact:
...
/// **The pair every one of the five scenes is pinned by**, written once because
/// it is one fact: `collection` is not declared, and the listing says which failure that is.
/// **The pair every one of the five scenes carries**, written once because it is
/// one fact: `collection` is declared, and the listing says which failure a missing one would be.
///
/// This is the `STANDS_ON_ITS_COMPONENTS` one ticket later. It was live as a *failing set*
...
/// The criterion said so — *every scene of that set is green except the wheel gate, which stays pinned
/// red* — and it turned by checking the shipped code rather than by
/// writing it.
///
...
/// from this crate at all; the re-export lifted it, and a barrier that has lifted is
...
/// **The pair both of the scenes stand on**, written once because it is one
/// fact: `tree` is declared and this screen draws through it.
///
/// It read `WAITING_FOR_TREE` once. The second entry is not decoration: the
...
/// **The pair both of the scenes are pinned by**, written once because it is one
...
/// **What both of the scenes are scenes of, and it is not declared yet.**
///
...
/// **What stands both of the scenes up**, written once because it is one fact:
/// `collapsible` is declared and the accordion draws *through* it.
///
/// It was `WAITING_FOR_COLLAPSIBLE` — the pair that pinned them — for a while, and the
...
/// The failing set both of the scenes **were** pinned in, until components 15
/// declared the subject.
///
/// One sentence and not two, because it was one fact: the screen was measured and green and the
/// component was missing. **Unlike the five, there was no second reason here** —
...
/// **What pins production the two, beyond the pair each shares with its file.**
///
...
/// belongs where the posted notch is driven. One pass put a component's three axes in one
...
const PINS_SCENE_37: &[Instrument] = &[
    ...
    // by nothing*; the band's slack answered the question and it fired, so it is now the
...
///
/// It was the *failing set* of three red rows for exactly one ticket, and criterion 7 is
...
///
/// **These three were `rehearsed_by` once** and they are `standing` now,
...
/// The overlay family's own file, which is where the scene is played.
...
/// only the subject: every count on this screen already reproduced the table.
///
/// Three of the seven columns do not reproduce and each says why where it belongs: `allocations`,
/// because the arena that made it zero was deleted; `content layers`, because
/// every request in the prototype carried a shadow layer `OverlayOpts` has no field for; and the
/// **menu delta**, because a menu collapses into a `Mode` of `collection` and a collection
...
/// The picture screen's own file, which is where the scene is played.
...
/// **The preview pane's own file**, which is where the three screens are played.
...
/// **What the three preview-pane scenes share**, whatever else stands each of them up:
...
/// readings printed of one product, the five offset spellings, the release argument, the
...
/// application's, because two defects have to be measured *on the assembled gallery* and a
...
///
/// The assembled gallery, and the number it was pinned with is **gone**: *9 956 cells nobody
...
/// exact set behind. What is on this list is the screen's own gates: the two halves of the partition rule as one
...
/// **The instruments** — a theme swap over the assembled gallery, played against an oracle.
///
/// The clause is *the memo-key rule has no caller; six panels keep the old palette permanently*, and both
...
/// The scene list, row for row, and this backlog's scenes beside it. **Forty-seven, of which
/// twenty-seven are the table and the table is the authority.**
///
/// The count in this sentence is a summary and never the gate:
/// `tests::the_specs_table_is_twenty_seven_rows_and_this_list_carries_them_all` reads the original's
/// rows off the spec file and asserts the split, and it caught this sentence going stale at 33.
/// **It had gone stale again, at thirty-six, and was corrected** — the note above is
...
pub const SCENES: [Scene; 47] = [
    Scene {
        ...
        // **Empty, and it was three entries until an earlier pass.** Everything that ran over
    ...
    Scene {
        ...
        // **The design states the two sizes in the same sentence as the number** — *0 of 24 000 cells
        // differ, at 300x80 and at 120x40* — and an earlier pass runs it at both. It is a
    ...
    Scene {
        ...
        // The a register row — *writes flat 1k -> 1M* — is green on it and the hit index is not.
        //
        // **Stood up by an earlier pass**, which is where `crate::listing::draw_into` stopped being a
    ...
    Scene {
        ...
        owed: false,
        from_a_survived_defect: true,
        // **The one scene of the five an earlier pass did not turn, and it stayed red for eight more
        // tickets.** Its failing set was the defect and not the missing subject, and the original's
        // criterion said so: *every scene of an earlier pass is green except the wheel gate, which stays
        // pinned red for an earlier pass.* an earlier pass stood it up over the shipped component with a
    ...
    Scene {
        ...
        owed: false,
        from_a_survived_defect: false,
        // **This row said `an earlier pass` and 16 did not own it.** Components the table is
        ...
        // owes a scene its subject; **An earlier pass is what stood it up.**
    ...
    Scene {
        ...
        owed: false,
        from_a_survived_defect: false,
        // **An earlier pass, and it is green *through* the subject.** `crate::accordion`'s
    ...
    Scene {
        ...
        // **`Screen` and not `Unstated`, and the design states it in the same sentence as the counts**:
        // *300x80, 312 chips, two selects, a menu bar; minimum of 60 steady frames.* Components
        // an earlier pass plays it at exactly that.
        size: Size::Screen { w: 300, h: 80 },
    ...
    Scene {
        ...
        covers: &[],
        stands: SCROLLING,
        // **Still `(owed)`, and the reason is the finding rather than the work.** the design marks this
        ...
        // per-row bitset that is exact by construction. What an earlier pass collected is the
        // number underneath it — the double write — and striking the mark here while the design still
        ...
        standing: Standing::Evaluated {
            by: &[
                ...
                // which is an earlier pass's first criterion from the instrument's side.
                Instrument::Unit {
                    file: AREA,
    ...
    Scene {
        ...
        // **Rehearsed as a count, and still `Unsubjected`.** an earlier pass measures the
        // repertoire axis in full and the colour axis not at all — `ColorDepth` is a barrier, which
        // a register row records — and a count over three rungs is not this screen on a terminal.
        rehearsed_by: &[
            Instrument::Unit {
                file: "crates/vitui-components/tests/glyph_matrix.rs",
    ...
    Scene {
        ...
        owed: false,
        from_a_survived_defect: false,
        // **Stood up by an earlier pass, and every figure it was pinned with reproduced.** The screen
    ...
    Scene {
        ...
        // **Claimed by an earlier pass**, which is what the empty list was waiting for: a scene that
        // is waiting for its subject is not yet evidence of anything, so `covers` stayed empty
        // while it was red. The design states both axes in its own words — *a landing is a shrink*, from
        ...
        // `an earlier pass`, whose own table is one scene and whose criteria never mention a
        // directory; an earlier pass stood the screen up and pinned it red on the subject; components
    ...
    Scene {
        ...
        // Green through the subject since an earlier pass.
    ...
    Scene {
        ...
        // through the subject since an earlier pass, and the key is now the component's parameter
    ...
    Scene {
        ...
        owed: false,
        from_a_survived_defect: false,
        // **Stood up by an earlier pass, and its own `inverted_by` had named an earlier pass.** The
        // gallery has existed since 39 and the scene went on saying *not played: `an earlier pass`
    ...
    Scene {
        ...
        owed: false,
        from_a_survived_defect: false,
        // **Stood up by an earlier pass, and what it took was the gate being rewritten.** The original's
        // clause here — *R20 the design has no caller; six panels keep the old palette permanently* — is
    ...
    //
    // **The first scene on this list that the design does not carry**, and it is not the last: `issues/11`
    ...
    Scene {
        ...
        content: Content::Assembled { parts: 338 },
        // **A shrink and not a resize.** the design refuses to bank the shrink gate written against a
    ...
    // **The second scene the design does not carry**, and the ticket that asked for it says so:
    ...
    Scene {
        ...
        // **A resize and not a shrink**, and that is the pairing the design sanctions rather than the
    ...
    Scene {
        ...
        // horizontal offset, against itself* is a register row, C04's, filed as a gate rather than
        // as a scene. The criterion asks for it as a scene, so it
        ...
        stands: TABLE,
        owed: false,
        // **False, and it is worth saying why.** the design marks three rows as existing because a defect
    ...
    // **The third scene the design does not carry**, and an earlier pass predicted it by name:
    ...
    Scene {
        ...
        stands: &[],
        owed: false,
        // Not a survived defect of the — it is a fixture the design states in one sentence — but it did
    ...
    // ── the scene, and the pair the design had no way to state ───────────────────────
    //
    // the design carries the wheel as **one** row, over one component, and one row is exactly what could
    // be written while the click was an arithmetic substitution: a delta added to an offset has no
    // second axis to be wrong on. An earlier pass posted a real notch and the pair appeared.
    //
    ...
    Scene {
        ...
        stands: &["scroll_area"],
        owed: false,
        // **Not the flag, and the distinction is the same one scene 31 draws.** the design marks three
    ...
    // ── production the three scenes, and they are the axes the design stated over one row ───────
    //
    // the design carries **one** `field` row — scene 13, the wrap memo at 300 and at 120 — and it is the
    ...
    Scene {
        ...
        stands: FIELD,
        owed: false,
        // **Not the flag**, and the distinction is the original's and scene 33's: the design marks three
    ...
    //
    // the design carries one `table` row and it is scene 7 — the twelve-column band under a horizontal
    ...
    // the production backlog's whole argument.
    //
    ...
    //
    // the design carries one overlay-family row and it is scene 14 — the family's five configurations,
    ...
    // a later pass gives: 06's two axes were decided by two different things — a screen and an
    ...
    // bar fixpoint and scene 18 is `Σ h` as the extent. Scene 33 is an earlier pass's wheel over an
    ...
    // substitution an earlier pass spent a ticket removing.
    Scene {
        number: 43,
        on_spec_table: false,
    ...
    // three glyphs off the row, and the partition this scene reads is the one the design always described:
    ...
    Scene {
        ...
        // **Four rigs and no residue**, which is why these are resizes and not shrinks: the design's
...
/// The dense screen's first criterion asks this question about all four.
...
///
/// An owed scene contributes nothing. A row is marked `(owed)` when the scene is normative and its
...
/// **Every scene on one line, in the column order**, so that a later ticket's numbers are
...
/// lines with `0.00 us  marked=0  writes=0` would be the first refinement arriving as a table — *a
...
mod tests {
    ...
    /// **Twenty-seven scenes, numbered 1..=27, once each — and the table is twenty-seven rows.**
    ///
    /// The count is the gate, and it is checked against the **spec file** rather than against a
    /// number typed here. A list normative by assertion is a list that drifts: *a scene is
    ...
    ///
    /// This is also where the *twenty-six* is settled. The criterion says twenty-six and the table
    ...
    /// **Every scene says what it decided, and says it in the figures.**
    ///
    ...
    /// one pass, then twenty-one, five and three; the collection was declared and
    /// `collection` and rewrote `crate::listing::draw_into` to draw through it, which moved four
    /// of the five red rows to `Evaluated` and left the fifth exactly where it was.
    ///
    /// `scroll_area`, `scrollbar` and `sticky` were declared, and that rewrote
    /// `crate::area`'s four screens to draw through them, which moved **all four** of its
    /// red rows to `Evaluated` — 11 red and 13 stood up became 7 and 17. The wheel gate stood
    ...
    /// the glossary forbids it and four *resolved* passes wrote it anyway. The collection's
    /// own criterion said so — *every scene of that set is green except the wheel gate, which stays
    /// pinned red* — and a single `inverted_by` across the five would have made that
    ...
    /// typed.**
    ///
    /// criterion 7, since inverted. [`crate::dense::subjects_declared`] opens the
    ...
    /// Sixteen of the thirty-four were claimed by the list; the other eighteen arrived one
    /// pass at a time — the backlog sliced one before each
    /// component's drawing ticket and *a scenes ticket is red on purpose until its component ticket
    /// lands*, and the last fourteen came in five passes, which is the lineage the `bare`
    /// assertion below spells out.
    ///
    /// **This heading was itself a stale count and was corrected**, reading *O5 moves
    /// from 34 of 34 to 11 of 34* — a sentence no build could have made true, left by a
    /// search-and-replace over the first number when something moved the second. It is the same
    /// defect the ticket was opened for, one file over, and it is why the count is asserted twice
    /// below rather than described once.
    ///
    /// The claim is narrow on purpose. A pair is claimed only where the row names the component
    ...
    fn seventeen_of_the_thirty_four_axis_obligations_have_a_scene_and_seventeen_do_not() {
        let evidence = axis_scenes();
        // **Twenty-nine since a later pass**, which claimed the overlay family's last four —
        // `select` scrolled and wheeled, `file_picker` shrunk and wheeled — all four inside a
        // layer, which is the one thing that decides them. It was twenty-five after a later pass,
        // which claimed `table`'s shrunk and wheeled (the design states one `table` row and it covers the
        // other two), twenty-three after a later pass, which claimed `field`'s scrolled, shrunk
        // and wheeled, and twenty after an earlier pass, which claimed the three pairs the three
        ...
        // a later pass renamed it, 08 renamed it again from thirty-six, and **09 a third time,
        // from forty**. Each time with its five register citations, which is the cost the policy
        // is priced against.
        //
        // **A later pass took the other arm on a name in `crate::obligations`**:
    ...
    ///
    /// criterion 1: *three scenes registered in the scene list, each
    ...
    fn scenes_for_a_component_lists_the_scenes_that_stand_it_up() {
        ...
        // **An earlier pass declared the subject and claims the pairs**, three of them.
        //
        // **And the two subjects stopped answering the same list in a later pass.** `file_picker`
    ...
    ///
    /// It was ten once. Two scenes lost theirs entirely and a third kept
    ...
    /// report a gate that nothing runs, which is the three-for-three finding from the other
    /// direction.
    ///
    /// **The three rows components 09 built are the case that made this sharper**, and components
    /// That is where the line was crossed *by the code moving rather than by the field moving*.
    ...
    ///
    /// The five are pinned red on instruments in `crate::listing` — the screen's
    ...
    fn a_rehearsal_is_never_what_stands_a_scene_up() {
        ...
        const SCREEN_FILES: [&str; 28] = [
            ...
            // an earlier pass: scenes 6 and 33 are played over the shipped `collection` and the shipped
            ...
            // an earlier pass: four of the gates are properties of `scroll_area`, `scrollbar` and
            ...
            // **The picture screen and its report** (an earlier pass). The screen's own file, the
            ...
            // **The preview pane's three screens and their report** (an earlier pass). Same
            ...
            PREVIEW,
            PREVIEW_NUMBERS,
            // **The assembled gallery and its report** (an earlier pass, stood up by 40). The screen
            ...
            GALLERY,
            GALLERY_NUMBERS,
            // **The memo census** (an earlier pass). The clause is *R20 the design has no caller*, and
            ...
            // **The 598-character form** (an earlier pass), which is the same defect over a value
            // that really is made of glyphs and at a corpus small enough to count by hand.
            "crates/vitui-components/tests/glyph_matrix.rs",
            // **The field's window** (a later pass). Scenes 34, 35 and 36 are played there over
            ...
            // axes.
            WINDOW,
            // **A later pass's**, and the overlay family's last four axes: the dropped list, where
            // both owners' bodies are drawn where a `Pen` can see them.
            DROPPED,
            // **A later pass's**, and the scroll family's last: the furniture an area writes and
            // its body does not. The file is the screen's own in exactly the sense `WINDOW` is —
            // `crate::area` carries this family's four the design rows and this one carries the two axes
            // they leave out.
            SURROUND,
        ];
        for scene in SCENES {
    ...
    ///
    /// *The surface after a shrink equals a freshly built one* is refused precisely because
    ...
    ///
    /// Criterion 7's shape, and the first refinement enforced on a *report*: twenty-seven rows of
    ...
    fn every_scene_reports_one_line_and_an_unplayed_one_says_so() {
        ...
        // **Scene 12 and not scene 11, since an earlier pass.** The measured line has to sit on a
        ...
        // Scene 12 is stood up since an earlier pass, and the label is the line's own rather than
        ...
        // have.** No row of `SCENES` is red since an earlier pass, and the check this replaces was
